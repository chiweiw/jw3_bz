...
- 在项目根目录执行构建命令：
  - `uv run skill-growth-report --input <源文本路径> --site-dir 报告站点 --db-path skill_report.db --jump-threshold 2.0`
- 说明：
  - `--input`：技能原文 `txt` 路径；默认 `1.txt`。
  - `--site-dir`：静态站点输出目录；默认 `报告站点`，可自定义。
  - `--db-path`：SQLite 数据库文件路径；默认 `skill_report.db`。
  - `--jump-threshold`：跃迁判定阈值（与差值中位数比较）；默认 `2.0`。
- 执行成功后，会生成/覆盖以下文件：
  - `报告站点/data/skills.json`
  - `报告站点/data/series.json`
  - `报告站点/data/values.json`
  - `报告站点/data/analysis.json`
  - 以及静态页面与资产：`报告站点/index.html`、`报告站点/charts.html`、`报告站点/assets/*`
- SQLite 数据库将写入四张表并可供后续检查：
  - `skills`、`series`、`values_tbl`、`analysis`

## 打开网页
- 直接在资源管理器中双击：
  - `报告站点/index.html`（基础信息页：筛选、排序、展开级次值与差值）
  - `报告站点/charts.html`（图表页：折线/差值曲线、线性高亮、跃迁点标记）
- 或使用任意静态服务器（可选），例如：
  - 在 `报告站点/` 目录启动静态服务后，浏览器访问对应地址。
